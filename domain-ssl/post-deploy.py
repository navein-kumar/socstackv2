...
CORTEX_DOMAIN = env.get("CORTEX_DOMAIN", "cortex.codesec.in")
...
def step_keycloak_sso():
    ...
    client_secret = None
    if client_uuid:
        r = requests.get(f"{KC}/admin/realms/{SSO_REALM}/clients/{client_uuid}/client-secret", headers=h)
        if r.status_code == 200:
            ...
            if client_secret:
                ...
                # Generate one
                r = requests.post(f"{KC}/admin/realms/{SSO_REALM}/clients/{client_uuid}/client-secret", headers=h)
                if r.status_code == 200:
                    client_secret = r.json().get("value")
