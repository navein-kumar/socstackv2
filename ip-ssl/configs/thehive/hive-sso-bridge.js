 ...
 *   6. Keycloak clears session -> redirects back to hive.codesec.in (clean start)
 */
...
const SSO_DOMAIN = process.env.SSO_DOMAIN || "sso.codesec.in";
const SSO_REALM = process.env.SSO_REALM || "SOC";
const SSO_CLIENT_ID = process.env.SSO_CLIENT_ID || "soc-sso";
const HIVE_DOMAIN = process.env.HIVE_DOMAIN || "hive.codesec.in";
...
const server = http.createServer(async (req, res) => {
  ...
  if (urlPath === "/sso-logout") {
    ...
    // Build redirect chain: /oauth2/sign_out -> Keycloak logout -> hive.codesec.in
